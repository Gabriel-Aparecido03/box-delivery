...
describe('Get status package - E2E', () => {
  ...
  test('[GET] package/near-of-me/:id', async () => {
    ...
    const packageOrder = makePackage({ deliveryManId: deliverman.id, recipientId: recipient.id, coordinates: deliverman.coordinate })
    await packageFactory.execute(packageOrder)

    const packageOrder2 = makePackage({ deliveryManId: deliverman.id, recipientId: recipient.id, coordinates: recipient.coordinate })
    await packageFactory.execute(packageOrder2)

    const accessToken = jwt.sign({ sub: data.id.toString() })

    const response = await request(app.getHttpServer())
      .get(`/package/near-of-me/${deliverman.id}`)
