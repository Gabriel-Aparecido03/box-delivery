...
import { MakePackagePickUpUseCase } from "./make-package-as-pick-up"
import { NotAllowed } from "./errors/not-allowed"

describe('Make package as delivered', () => {
  let sut: MakePackagePickUpUseCase
  ...
  beforeEach(() => {
    ...
    sut = new MakePackagePickUpUseCase(inMemoryDeliverymanRepository, inMemoryPackageRepository)
  ...
  it('should be to make packages as delivered', async () => {
    ...
    expect(inMemoryPackageRepository.items[0].status.value.status).toEqual('pick-up')
  })

  it('not should be to make packages as delivered when the deliverymanId dont match of package deliverymanId', async () => {
    const deliveryman = makeDeliveryman()
    inMemoryDeliverymanRepository.items.push(deliveryman)
