...
interface MakePackageReturnedUseCaseTypeProps {
  packageId: string
  deliverymanId: string
}

@Injectable()
export class MakePackageReturnedUseCase {
  constructor(private deliverymanRepository: DeliverymanRepository, private packageRepository: PackageRepository) { }

  async execute({ deliverymanId, packageId }: MakePackageReturnedUseCaseTypeProps) {
    ...
    await this.packageRepository.changeStatusReturned(Package.create({
